...
/**
 *  <p>This interface encapsulates the configuration of ClientAuthContext objects for a message layer and application
 *  context (for example, the messaging context of a specific application, or set of applications).</p>
 *
 *  <p>Implementations of this interface are returned by an AuthConfigProvider.</p>
 *
 *  <p>Callers interact with a ClientAuthConfig to obtain ClientAuthContext objects suitable for processing a given
 *  message exchange at the layer and within the application context of the ClientAuthConfig. Each
 *  ClientAuthContext object is responsible for instantiating, initializing, and invoking the one or more
 *  ClientAuthModules encapsulated in the ClientAuthContext.</p>
 *
 *  <p>After having acquired a ClientAuthContext, a caller operates on the context to cause it to invoke the
 *  encapsulated ClientAuthModules to secure client requests and to validate server responses.</p>
 *  
...
public interface ClientAuthConfig extends AuthConfig
{
    ...
    * associated with the given authContextID.</p>
    *
    * <p>Specifically, this method accesses this ClientAuthConfig object with the argument authContextID to
    * determine the ClientAuthModules that are to be encapsulated in the returned ClientAuthContext instance.</p>
    *
    * <p>The ClientAuthConfig object establishes the request and response MessagePolicy objects that are passed to
    * the encapsulated modules when they are initialized by the returned ClientAuthContext instance. It is the
    * modules’ responsibility to enforce these policies when invoked.</p>
    * 
    ...
    * @param clientSubject A Subject that represents the source of the service request to be secured by the
    ...
    *                      this parameter.
    * @param properties A Map object that may be used by the caller to augment the properties that will be
